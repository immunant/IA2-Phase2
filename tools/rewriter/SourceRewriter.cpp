...
int main(int argc, const char **argv) {
  ...
  for (int caller_pkey = 0; caller_pkey < num_pkeys; caller_pkey++) {
    ...
    for (const auto &fn_name : undefined_fns) {
      ...
      // Skip ld.so functions here - they are handled later by the dedicated ldso
      // autowrap block which adds them to multicaller_fns with proper _from_N
      // suffixes. Processing them here would generate unsuffixed --wrap flags
      // that conflict with the suffixed versions generated by the multicaller
      // pipeline.
      if (gLibcCompartmentEnabled &&
