...
static auto abiSlotsForArg(const clang::QualType& qt, const clang::CodeGen::ABIArgInfo& argInfo, const clang::ASTContext& astContext) -> std::vector<CAbiArgKind> {
	...
	switch(argInfo.getKind()) {
		...
		case IndirectAliased: /* ptr in register */
		{
			...
			if (rec != nullptr) {
				const clang::RecordDecl* decl = rec->getDecl();
				const clang::ASTRecordLayout& layout = astContext.getASTRecordLayout(decl);
				...
				/* aggregates of 1-8 eightbytes have each eightbyte classified individually.
				we have to iterate over fields to determine which fields are present in each
				eightbyte, then resolve conflicts with the logic from SysV ABI §3.3.3 (step 4): */
				...
				/* 5c (n.b. that we ignore the sseup condition as we do not yet handle sseup) */
		...
		case Direct: /* in register */
		{
			...
			if(STy) {
				if (argInfo.getCanBeFlattened()) {
					...
					return {CAbiArgKind::Integral}; /* one pointer in register, see `clang's ClangToLLVMArgMapping::construct` */
				}
			}
		...
		case InAlloca: /* via implicit pointer */
			return {};
		case Expand: /* split aggregate into multiple registers -- already handled before our logic runs? */
...
const clang::CodeGen::CGFunctionInfo& cgFunctionInfo(clang::CodeGen::CodeGenModule& cgm, const clang::FunctionDecl& fnDecl) {
	...
	if (ft->isFunctionProtoType()) {
		return clang::CodeGen::arrangeFreeFunctionType(cgm, canQualFnTy.castAs<clang::FunctionProtoType>());
	} else {
		return clang::CodeGen::arrangeFreeFunctionType(cgm, canQualFnTy.castAs<clang::FunctionNoProtoType>());
	}
}
...
auto determineAbiForDecl(const clang::FunctionDecl& fnDecl) -> CAbiSignature {
	...
	clang::CodeGenerator* codeGenerator = CreateLLVMCodeGen(astContext.getDiagnostics(),
		llvm::StringRef(),
		hso, ppo, cgo, llvmCtx);
	...
	/* our ABI computation assumes SysV ABI; bail if another ABI is explicitly selected */
	if(convention != clang::CallingConv::CC_X86_64SysV && convention != clang::CallingConv::CC_C) {
		printf("calling convention of %s was not SysV (%d)\n", name.c_str(), info.getASTCallingConvention());
		abort();
	}
