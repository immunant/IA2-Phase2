...
ngx_output_chain(ngx_output_chain_ctx_t *ctx, ngx_chain_t *in)
{
       ...
       )
    {
        ...
        if (in == NULL) {
            return IA2_CALL(ctx->output_filter, 5, 1)(ctx->filter_ctx, in);
        }
    ...
    for ( ;; ) {
        ...
        last = IA2_CALL(ctx->output_filter, 5, 1)(ctx->filter_ctx, out);
...
ngx_chain_writer(void *data, ngx_chain_t *in)
{
    ...
    chain = IA2_CALL(c->send_chain, 6, 1)(c, ctx->out, ctx->limit);
