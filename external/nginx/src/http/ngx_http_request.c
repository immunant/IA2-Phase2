...
ngx_http_header_t  ngx_http_headers_in[] = {
    { ngx_string("Host"), offsetof(ngx_http_headers_in_t, host),
                 IA2_FN(ngx_http_process_host) },

    { ngx_string("Connection"), offsetof(ngx_http_headers_in_t, connection),
                 IA2_FN(ngx_http_process_connection) },

    { ngx_string("If-Modified-Since"),
                 offsetof(ngx_http_headers_in_t, if_modified_since),
                 IA2_FN(ngx_http_process_unique_header_line) },
    ...
    { ngx_string("User-Agent"), offsetof(ngx_http_headers_in_t, user_agent),
                 IA2_FN(ngx_http_process_user_agent) },

    { ngx_string("Referer"), offsetof(ngx_http_headers_in_t, referer),
                 IA2_FN(ngx_http_process_header_line) },
    ...
    { ngx_string("X-Forwarded-For"),
                 ...
                 IA2_FN(ngx_http_process_multi_header_lines) },
#endif
...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    c->log->handler = IA2_FN(ngx_http_log_error);
    c->log->data = ctx;
    ...
    rev = c->read;
    rev->handler = IA2_FN(ngx_http_wait_request_handler);
    c->write->handler = IA2_FN(ngx_http_empty_handler);
...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    n = IA2_CALL(c->recv, 22, 1)(c, b->last, size);
    ...
    rev->handler = IA2_FN(ngx_http_process_request_line);
...
ngx_http_alloc_request(ngx_connection_t *c)
{
    ...
    r->log_handler = IA2_FN(ngx_http_log_error_handler);
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {
            ...
            rev->handler = IA2_FN(ngx_http_process_request_headers);
...
ngx_http_read_request_header(ngx_http_request_t *r)
{
    ...
    if (rev->ready) {
        n = IA2_CALL(c->recv, 22, 1)(c, r->header_in->last,
...
ngx_http_process_request(ngx_http_request_t *r)
{
    ...
    c->read->handler = IA2_FN(ngx_http_request_handler);
    c->write->handler = IA2_FN(ngx_http_request_handler);
    r->read_event_handler = IA2_FN(ngx_http_block_reading);
...
ngx_http_request_handler(ngx_event_t *ev)
{
    ...
    if (ev->write) {
        ...
        IA2_CALL(r->read_event_handler, 42, 1)(r);
    }
...
ngx_http_finalize_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    if (r != r->main && r->post_subrequest) {
        rc = IA2_CALL(r->post_subrequest->handler, 43, 1)(r, r->post_subrequest->data, rc);
    }
    ...
    if (r != r->main) {
        ...
        if (r == c->data || r->background) {
            ...
            r->write_event_handler = IA2_FN(ngx_http_request_finalizer);
...
ngx_http_terminate_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    while (cln) {
        if (cln->handler.ptr) {
            IA2_CALL(cln->handler, 0, 1)(cln->data);
        }
    ...
    if (mr->write_event_handler.ptr) {
        ...
        mr->write_event_handler = IA2_FN(ngx_http_terminate_handler);
...
ngx_http_set_write_handler(ngx_http_request_t *r)
{
    ...
    r->read_event_handler = r->discard_body ?
                                IA2_FN(ngx_http_discarded_request_body_handler):
                                IA2_FN(ngx_http_test_reading);
    r->write_event_handler = IA2_FN(ngx_http_writer);
...
ngx_http_block_reading(ngx_http_request_t *r)
{
    ...
    {
        if (IA2_CALL(ngx_del_event, 11, 1)(r->connection->read, NGX_READ_EVENT, 0) != NGX_OK) {
            ngx_http_close_request(r, 0);
        }
    }
}
...
ngx_http_test_reading(ngx_http_request_t *r)
{
    ...
    if ((ngx_event_flags & NGX_USE_LEVEL_EVENT) && rev->active) {
        ...
        if (IA2_CALL(ngx_del_event, 11, 1)(rev, NGX_READ_EVENT, 0) != NGX_OK) {
            ngx_http_close_request(r, 0);
        }
    }
...
ngx_http_set_keepalive(ngx_http_request_t *r)
{
    ...
    wev = c->write;
    wev->handler = IA2_FN(ngx_http_empty_handler);
    ...
    rev->handler = IA2_FN(ngx_http_keepalive_handler);

    if (wev->active && (ngx_event_flags & NGX_USE_LEVEL_EVENT)) {
        if (IA2_CALL(ngx_del_event, 11, 1)(wev, NGX_WRITE_EVENT, 0) != NGX_OK) {
            ngx_http_close_connection(c);
            return;
        }
    }
...
ngx_http_keepalive_handler(ngx_event_t *rev)
{
    ...
    c->log->handler = (typeof(c->log->handler)) { NULL };
...
ngx_http_set_lingering_close(ngx_connection_t *c)
{
    ...
    rev = c->read;
    rev->handler = IA2_FN(ngx_http_lingering_close_handler);
...
ngx_http_lingering_close_handler(ngx_event_t *rev)
{
    ...
    do {
        n = IA2_CALL(c->recv, 22, 1)(c, buffer, NGX_HTTP_LINGERING_BUFFER_SIZE);
...
ngx_http_log_error(ngx_log_t *log, u_char *buf, size_t len)
{
    ...
    if (r) {
        return IA2_CALL(r->log_handler, 44, 1)(r, ctx->current_request, p, len);
...
IA2_DEFINE_WRAPPER_ngx_http_keepalive_handler
IA2_DEFINE_WRAPPER_ngx_http_lingering_close_handler
IA2_DEFINE_WRAPPER_ngx_http_log_error
IA2_DEFINE_WRAPPER_ngx_http_log_error_handler
IA2_DEFINE_WRAPPER_ngx_http_process_connection
IA2_DEFINE_WRAPPER_ngx_http_process_header_line
IA2_DEFINE_WRAPPER_ngx_http_process_host
IA2_DEFINE_WRAPPER_ngx_http_process_multi_header_lines
IA2_DEFINE_WRAPPER_ngx_http_process_request_headers
IA2_DEFINE_WRAPPER_ngx_http_process_request_line
IA2_DEFINE_WRAPPER_ngx_http_process_unique_header_line
IA2_DEFINE_WRAPPER_ngx_http_process_user_agent
IA2_DEFINE_WRAPPER_ngx_http_request_finalizer
IA2_DEFINE_WRAPPER_ngx_http_request_handler
IA2_DEFINE_WRAPPER_ngx_http_terminate_handler
IA2_DEFINE_WRAPPER_ngx_http_wait_request_handler
IA2_DEFINE_WRAPPER_ngx_http_writer
