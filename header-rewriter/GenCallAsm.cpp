...
public:
  ...
  bool is_stack() { return reg == nullptr; }
  bool is_xmm() {
  ...
  operator const char *() { return as_str(); }
};
...
auto emit_call_asm(const CAbiSignature &sig, const std::string &name, int pkey)
  ...
  // add_asm_line(ss, "mov rax, [ia2_trusted_stackptr]"); // use rax to point at
  // the trusted stack
