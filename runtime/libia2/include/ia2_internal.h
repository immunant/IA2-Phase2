...
#define PKRU_SAVE_AND_RESTORE(body, max)                                       \
    __asm__ goto volatile(                                                     \
        ...
        /* If we get here, we have an unexpected PKRU value. */                \
        "ud2\n"                                                                \
        :                                                                      \
        :                                                                      \
        : "rax", "rcx", "rdx", "cc"                                            \
        : REPEATB(max, PKRU_LABEL, PKRU_LABEL_NO_COMMA));                      \
...
#define _IA2_INIT_RUNTIME(n)                                                   \
  ...
  __attribute__((weak)) void init_stacks_and_setup_tls(void) {                 \
    verify_tls_padding();                                                      \
    PKRU_SAVE_AND_RESTORE(REPEATB(n, ALLOCATE_COMPARTMENT_STACK_AND_SETUP_TLS, nop_macro), n); \
