...
// This variable is used in the IA2_CALL macro generated by the rewriter to
// simplify the problem of reading the function pointer expression from the
// stack in assembly. Each time the wrapper defined by IA2_CALL is called, the
// function pointer expression is stored in ia2_fn_ptr then the call gate
// wrapper reads this variable to determine the function to call.
// TODO: Ideally we'd remove this and read directly from the stack in the call
// gate wrapper, but if is too difficult to implement in a realiable way then
// this variable should be thread-local. There should also be one per
// compartment for security
void *ia2_fn_ptr IA2_SHARED_DATA = NULL;
