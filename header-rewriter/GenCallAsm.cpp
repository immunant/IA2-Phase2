...
static void append_arg_kinds(std::stringstream& ss, std::vector<CAbiArgKind> args) {
	bool first = true;
	for (auto arg : args) {
		if (!first) {
			ss << ", ";
...
auto emit_call_asm(const CAbiSignature& sig, const std::string& name) -> std::string {
	...
	if (reg_arg_count > 0) {
		add_comment_line(ss, "push return regs to untrusted stack to preserve them while changing pkru");
	}
