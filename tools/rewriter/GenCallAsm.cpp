...
std::string emit_wrapper(FnSignature &sig,
  ...
  std::string wrapper = emit_asm_wrapper(sig.abi, asm_wrapper_name, target_name, kind, caller_pkey, target_pkey, arch, as_macro);
  wrapper += "\n";
  wrapper += emit_c_wrapper(sig.api, wrapper_name, as_macro);
  wrapper += "\n\n";
  return wrapper;
}
