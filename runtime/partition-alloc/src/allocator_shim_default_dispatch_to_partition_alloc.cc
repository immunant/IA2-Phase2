...
LeakySingleton<partition_alloc::PartitionRoot, MainPartitionConstructor>
    g_root[partition_alloc::internal::kNumCompartments - 1] PA_CONSTINIT = {};
...
LeakySingleton<partition_alloc::PartitionRoot, SharedPartitionConstructor>
    g_shared_root PA_CONSTINIT = {};

partition_alloc::PartitionRoot *Allocator() {
  size_t pkey = ::ia2_get_pkey();
  if (pkey == 0) {
    return g_shared_root.Get();
  }
  return g_root[pkey - 1].Get();
}
...
partition_alloc::PartitionRoot *SharedAllocator() {
  return g_shared_root.Get();
}
...
// static
partition_alloc::PartitionRoot* PartitionAllocMalloc::SharedAllocator() {
...
uint32_t GetMainPartitionRootExtrasSize() {
  ...
  return Allocator()->settings.extras_size;
#else
  return 0;
